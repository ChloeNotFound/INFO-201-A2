...
state_highest_ratio <- states %>% 
  ...
  filter(death_ratio == max(death_ratio,na.rm = TRUE)) %>% 
  pull(state)
...
date_most_cases<-national %>%
  filter(new_case==max(new_case,na.rm=TRUE)) %>%
  pull(date)

# What was the date when the most new deaths occured?
# `date_most_deaths`
date_most_deaths <- national[national$new_death == max(national$new_death,na.rm = TRUE), 1][2]
print(date_most_deaths)

# How many people died on the date when the most deaths occured? `most_deaths`
date_most_cases <- national[national$new_case == max(national$new_case, na.rm = TRUE), 1][2]
...
state_by_day<-states %>% 
  group_by(date) %>% 
  summarise(state_total = sum(cases, na.rm = TRUE))
...
county_by_day <- counties %>% 
  group_by(date) %>% 
  summarise(county_total = sum(cases, na.rm = TRUE))
...
num_state_diff <- all_totals %>% 
  mutate(diff1 = cases - state_total != 0) %>% 
  pull(diff1) %>% 
  sum()
...
num_county_diff <- all_totals %>% 
  mutate(diff2 = cases - county_total != 0) %>% 
  pull(diff2) %>% 
  sum()
...
sum_county_to_state <- counties %>%
  group_by(state, date) %>%
  summarize(county_totals = sum ( cases ,na.rm = TRUE), .groups = "drop")

# Then, let's join together the `sum_county_to_state` dataframe with the
# `states` dataframe into the variable `joined_states`.
joined_states <- left_join(states , sum_county_to_state, by = c("date","state"))
...
state_highest_difference <- has_discrepancy %>%
  mutate ( highest_absolute_difference = abs(cases - county_totals)) %>%
  filter (highest_absolute_difference == max( highest_absolute_difference)) %>%
  pull(state)
...
# 1:  What is the name of the location (county, state) with the lowest number
# of deaths? `location_least_deaths`
location_least_deaths <- counties[counties$deaths == min(counties$deaths), 3]

#2. # Which state has had the lowest number of cases?
# `state_lowest_cases`
lowest_num_cases <- min(states$cases)
state_lowest_cases <- states[states$cases == lowest_num_cases, 2,]

#3. Which county has had the lowest number of cases?
# `county_lowest_cases`
county_lowest_cases <- counties[counties$date == min(counties$date), 2]
